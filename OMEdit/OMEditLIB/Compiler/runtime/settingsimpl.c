...
// #include "../../SimulationRuntime/c/util/omc_msvc.h"
...
const char* SettingsImpl__getInstallationDirectoryPath(void) {
  ...
  // 首先尝试使用 /proc/self/exe 获取完整路径
  char exe_path[PATH_MAX];
  ssize_t len = readlink("/proc/self/exe", exe_path, sizeof(exe_path) - 1);
  
  if (len != -1) {
    exe_path[len] = '\0';
    fprintf(stderr, "DEBUG: readlink returned path: %s\n", exe_path);
    omc_installationPath = omc_alloc_interface.malloc_strdup(exe_path);
  } else {
    // 如果 readlink 失败，回退到 dladdr
    Dl_info info;
    if (!dladdr((void*) SettingsImpl__getInstallationDirectoryPath, &info)) {
      fprintf(stderr, "dladdr() failed: %s\n", strerror(errno));
      exit(EXIT_FAILURE);
    } else {
      fprintf(stderr, "DEBUG: dladdr returned path: %s\n", info.dli_fname);
      omc_installationPath = omc_alloc_interface.malloc_strdup(info.dli_fname);
    }
  }
  
  // 添加调试信息显示处理前的路径
  fprintf(stderr, "DEBUG: Before stripbinpath: %s\n", omc_installationPath);
  
  stripbinpath(omc_installationPath);
  
  // 添加调试信息显示处理后的路径
  fprintf(stderr, "DEBUG: After stripbinpath: %s\n", omc_installationPath);

  if (!(omc_installationPath && omc_installationPath[0])) {
    fprintf(stderr, "Failed to get binary path from executable path\n");
    exit(EXIT_FAILURE);
  }
  // Dl_info info;
  // // int erron = 0;
