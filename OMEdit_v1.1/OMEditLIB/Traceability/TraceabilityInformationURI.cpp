...
#ifndef OM_DISABLE_GIT
#include "Git/GitCommands.h"  // 注释掉这行，因为Git文件夹已被禁用
#endif
...
void TraceabilityInformationURI::translateModelCreationURIToJsonMessageFormat(QString modelingActivity, QString toolURI, QString activityURI, QString agentURI, QString fileNameURI, QString entityType, QString path, QString gitHash)
{
  ...
  QString jsonMessageFormat = QString("{\"rdf:RDF\" : {\n"
                                      ...
                                      "            }\n"
                                      "     ]\n"
                                      "}}").arg(fileNameURI.simplified()).arg(activityURI.simplified()).arg(agentURI.simplified()).arg(toolURI.simplified()).arg(userName).arg(email).arg(modelingActivity).arg(entityType.simplified()).arg(path.simplified()).arg(gitHash.isEmpty() ? "unknown" : gitHash.simplified());
...
void TraceabilityInformationURI::translateURIToJsonMessageFormat(QString modelingActivity, QString toolURI, QString activityURI, QString agentURI, QString sourceModelFileNameURI, QString fmuFileNameURI, QString entityType, QString path, QString gitHash)
{
  ...
  QString jsonMessageFormat = QString("{\"rdf:RDF\" : {\n"
                                      ...
                                      "            \"prov:used\": {\"prov:Entity\": [ {\"rdf:about\": \"%6\"}]},\n"
                                      "            \"prov:generated\": {\"prov:Entity\": [ {\"rdf:about\": \"%3\"}]},\n"
                                      "            \"rdf:about\": \"%5\"\n"
                                      "            }\n"
                                      "     ]\n"
                                      "}}").arg(modelingActivity.simplified()).arg(toolURI.simplified()).arg(fmuFileNameURI.simplified()).arg(agentURI.simplified()).arg(activityURI.simplified()).arg(sourceModelFileNameURI.simplified()).arg(userName).arg(email).arg(entityType.simplified()).arg(path.simplified()).arg(gitHash.isEmpty() ? "unknown" : gitHash.simplified());
  sendTraceabilityInformation(jsonMessageFormat);
}
